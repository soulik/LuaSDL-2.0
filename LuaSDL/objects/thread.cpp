...
static LuaSDL::LuaThread *getself (lutok::state& state) {
	LuaSDL::LuaThread *p;
...
static void movevalues (lutok::state *send, lutok::state *rec) {
	int n = send->get_top();
	int i;
...
static int ll_start (lutok::state& state) {
	SDL_Thread * thread;
	if (state.is_string(1)){
		lutok::state * L1 = state.newState();
		if (L1 == NULL)
			state.error("unable to create new state");

		try{
			L1->load_string(state.to_string(1));
		}catch(lutok::api_error & error){
